...
from decorators.decorators import log_action_start
...
@log_action_start(filename=user_log_filename, message="SOME USER IS TRYING TO LOGIN")
...
def read_countries(request: Request, skip: int = 0, limit: int = 10, db: Session = Depends(get_db),
                   username: str = Depends(TokenVerifier.verify_token)):
...
def read_country(request: Request, country_id: int, db: Session = Depends(get_db),
                 username: str = Depends(TokenVerifier.verify_token)):
...
def create_country(request: Request, country: schemas.CountryCreate, db: Session = Depends(get_db),
                   username: str = Depends(TokenVerifier.verify_token)):
...
def update_country(request: Request, country_id: int, country: schemas.CountryCreate, db: Session = Depends(get_db),
                   username: str = Depends(TokenVerifier.verify_token)):
...
def delete_country(request: Request, country_id: int, db: Session = Depends(get_db),
                   username: str = Depends(TokenVerifier.verify_token)):
...
def read_cities(request: Request, skip: int = 0, limit: int = 10, db: Session = Depends(get_db),
                username: str = Depends(TokenVerifier.verify_token)):
...
def read_city(request: Request, city_id: int, db: Session = Depends(get_db),
              username: str = Depends(TokenVerifier.verify_token)):
...
def create_city(request: Request, city: schemas.CityCreate, db: Session = Depends(get_db),
                username: str = Depends(TokenVerifier.verify_token)):
...
def update_city(request: Request, city_id: int, city: schemas.CityCreate, db: Session = Depends(get_db),
                username: str = Depends(TokenVerifier.verify_token)):
...
def read_airports(request: Request, skip: int = 0, limit: int = 10, db: Session = Depends(get_db),
                  username: str = Depends(TokenVerifier.verify_token)):
...
def read_airport(request: Request, airport_id: int, db: Session = Depends(get_db),
                 username: str = Depends(TokenVerifier.verify_token)):
...
def read_airports_by_country(request: Request, skip: int = 0, limit: int = 10, country_id: int = None,
                             db: Session = Depends(get_db), username: str = Depends(TokenVerifier.verify_token)):
...
def create_airport(request: Request, airport: schemas.AirportCreate, db: Session = Depends(get_db),
                   username: str = Depends(TokenVerifier.verify_token)):
...
def update_airport(request: Request, airport_id: int, airport: schemas.AirportCreate, db: Session = Depends(get_db),
                   username: str = Depends(TokenVerifier.verify_token)):
...
def delete_airport(request: Request, airport_id: int, db: Session = Depends(get_db),
                   username: str = Depends(TokenVerifier.verify_token)):
...
def read_plane(request: Request, plane_id: int, db: Session = Depends(get_db),
               username: str = Depends(TokenVerifier.verify_token)):
...
def read_planes(request: Request, skip: int = 0, limit: int = 10, db: Session = Depends(get_db),
                username: str = Depends(TokenVerifier.verify_token)):
...
def create_plane(request: Request, plane: schemas.PlaneCreate, db: Session = Depends(get_db),
                 username: str = Depends(TokenVerifier.verify_token)):
...
def update_plane(request: Request, plane_id: int, plane: schemas.PlaneUpdate, db: Session = Depends(get_db),
                 username: str = Depends(TokenVerifier.verify_token)):
...
def delete_plane(request: Request, plane_id: int, db: Session = Depends(get_db),
                 username: str = Depends(TokenVerifier.verify_token)):
...
def read_flights(request: Request, skip: int = 0, limit: int = 10, db: Session = Depends(get_db),
                 username: str = Depends(TokenVerifier.verify_token)):
...
def read_flight(request: Request, flight_id: int, db: Session = Depends(get_db),
                username: str = Depends(TokenVerifier.verify_token)):
...
def create_flight(request: Request, flight: schemas.FlightCreate, db: Session = Depends(get_db),
                  username: str = Depends(TokenVerifier.verify_token)):
...
def update_flight(request: Request, flight_id: int, flight: schemas.FlightUpdate, db: Session = Depends(get_db),
                  username: str = Depends(TokenVerifier.verify_token)):
...
def delete_flight(request: Request, flight_id: int, db: Session = Depends(get_db),
                  username: str = Depends(TokenVerifier.verify_token)):
...
def read_ticket(request: Request, ticket_id: int, db: Session = Depends(get_db),
                username: str = Depends(TokenVerifier.verify_token)):
...
def read_tickets(request: Request, skip: int = 0, limit: int = 10, db: Session = Depends(get_db),
                 username: str = Depends(TokenVerifier.verify_token)):
...
def update_ticket(request: Request, ticket_id: int, ticket: schemas.TicketCreate, db: Session = Depends(get_db),
                  username: str = Depends(TokenVerifier.verify_token)):
...
def read_available_tickets(request: Request, plane_name: str = "", skip: int = 0, limit: int = 10,
                           db: Session = Depends(get_db), username: str = Depends(TokenVerifier.verify_token)):
...
def reserve_ticket(request: Request, ticket_id: int, db: Session = Depends(get_db),
                   username: str = Depends(TokenVerifier.verify_token)):

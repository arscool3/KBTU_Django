...
Revision ID: 07e0a9b3d1d5
Revises: 43e3b2a2c4f2
Create Date: 2024-05-11 17:54:31.717661
...
revision: str = '07e0a9b3d1d5'
down_revision: Union[str, None] = '43e3b2a2c4f2'
...
def upgrade() -> None:
    ...
    sa.Column('session_finished_at', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['auth_id'], ['public.authorization.id'], ),
    sa.PrimaryKeyConstraint('id'),
    schema='service'
    )
    op.create_index(op.f('ix_service_session_auth_id'), 'session', ['auth_id'], unique=False, schema='service')
...
def downgrade() -> None:
    ...
    op.drop_constraint(None, 'application', schema='public', type_='foreignkey')
    op.drop_constraint(None, 'application', schema='public', type_='foreignkey')
    op.create_foreign_key('application_manager_id_fkey', 'application', 'authorization', ['manager_id'], ['id'])
    ...
    op.drop_index(op.f('ix_service_session_auth_id'), table_name='session', schema='service')
    op.drop_table('session', schema='service')
    # ### end Alembic commands ###
